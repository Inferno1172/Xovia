(() => {
    ...
    function setCharacterState(character, state) {
      ...
      if (chair){ chair.classList.remove('speaking','active'); }
      if (state && state !== 'idle') { blob.classList.add(`state-${state}`); }
      if (chair){
    ...
    function scrollTimeline(){
        try{ timeline.scrollTop = timeline.scrollHeight; } catch {}
    }
    ...
    function setDisabled(v){
        t.disabled = v; sendBtn.disabled = v;
    ...
    async function ensureSession(){
        if (sessionId) return;
        const r = await fetchWithRetry(BASE + "/api/session", {method:"POST", headers:{'Content-Type':'application/json'}, body: JSON.stringify({})});
    ...
    async function handleSend(){
        ...
        try{
            ...
            if (j.crisis){
                if (typingRow){ typingRow.row.remove(); typingRow.tag.remove(); }
            ...
            if (j.awaitMore) {
                ...
                // if going to SELF: show suggestions. if going to MONSTER: hide them
    ...
    t.addEventListener("keydown", e=>{ if (e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); handleSend(); } });
    t.addEventListener("input", ()=>{t.style.height='auto';t.style.height=Math.min(t.scrollHeight,200)+'px';updateStackHeight();});
